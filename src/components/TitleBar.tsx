...
import { platformAPI, isTauri, isAndroid } from '@/utils/platform'
...
function TitleBar() {
  ...
  const handleMinimize = () => {
    if (platformAPI.minimize) {
      platformAPI.minimize()
    }
  }

  const handleClose = () => {
    const { closeAction } = useSettingsStore.getState()
    if (closeAction === 'hide' && platformAPI.hide) {
      platformAPI.hide()
    } else if (platformAPI.close) {
      platformAPI.close()
    }
  }
